...
const checkVisibility = async (ownerId, requesterId, setting) => {
...
export const getUserStats = async (req, res) => {
    try {
        ...
        if (!user)
            return res.status(404).json({ message: 'User not found' });
        const canSee = await checkVisibility(user.id, requesterId, user.privacySettings?.showStats || 'PUBLIC');
        if (!canSee)
            return res.status(403).json({ message: 'Private section' });
...
export const getUserFingerprint = async (req, res) => {
    try {
        ...
        if (!user)
            return res.status(404).json({ message: 'User not found' });
        const canSee = await checkVisibility(user.id, requesterId, user.privacySettings?.showFingerprint || 'PUBLIC');
        if (!canSee)
            return res.status(403).json({ message: 'Private section' });
        ...
        const averages = categories.map(cat => ({
            ...
            average: Number((ratings.reduce((acc, r) => acc + r[cat], 0) / ratings.length).toFixed(1))
        }));
...
export const getUserTopTen = async (req, res) => {
    try {
        const { username } = req.params;
        const requesterId = req.userId;
        const user = await prisma.user.findUnique({
        ...
        if (!user)
            return res.status(404).json({ message: 'User not found' });
        const canSee = await checkVisibility(user.id, requesterId, user.privacySettings?.showTopTen || 'PUBLIC');
        if (!canSee)
            return res.status(403).json({ message: 'Private section' });
...
export const getUserActivity = async (req, res) => {
    try {
        ...
        if (!user)
            return res.status(404).json({ message: 'User not found' });
        const canSee = await checkVisibility(user.id, requesterId, user.privacySettings?.showActivity || 'PUBLIC');
        if (!canSee)
            return res.status(403).json({ message: 'Private section' });
...
export const addTopTenEntry = async (req, res) => {
    try {
        const userId = req.userId;
...
export const removeTopTenEntry = async (req, res) => {
    try {
        const userId = req.userId;
...
export const removeTopTenEntryByRank = async (req, res) => {
    try {
        const userId = req.userId;
