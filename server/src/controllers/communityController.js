...
export const browseCommunities = async (req, res) => {
    try {
...
export const leaveCommunity = async (req, res) => {
    try {
        ...
        if (member.role === 'ADMIN')
            return res.status(400).json({ error: 'Admins cannot leave. Transfer ownership or delete the community.' });
        await prisma.$transaction([
