...
const getStoryWithAccess = async (storyId, userId) => {
    ...
    if (!story)
        return { allowed: false, error: 'Story not found', status: 404 };
...
export const browseStories = async (req, res) => {
    try {
...
export const updateStory = async (req, res) => {
    try {
        ...
        if (isPublished === 'true' && story._count.chapters === 0) {
...
export const addCollaborator = async (req, res) => {
    try {
        ...
        if (story.ownerId !== userId)
            return res.status(403).json({ error: 'Forbidden' });
        const userToAdd = await prisma.user.findUnique({ where: { username: username } });
        if (!userToAdd)
...
export const removeCollaborator = async (req, res) => {
    try {
        ...
        await prisma.storyCollaborator.delete({
            where: { storyId_userId: { storyId: id, userId: collaboratorId } }
        });
        res.status(204).send();
    }
    catch (error) {
...
export const rateStory = async (req, res) => {
    try {
        ...
        if (!story)
            return res.status(404).json({ error: 'Story not found' });
        if (story.ownerId === userId)
            return res.status(400).json({ error: 'Cannot rate your own story' });
        const rating = await prisma.storyRating.upsert({
            where: { storyId_userId: { storyId: id, userId } },
            update: { score: Number(score), review: review },
            create: { storyId: id, userId, score: Number(score), review: review }
        });
        res.json(rating);
    }
    catch (error) {
...
export const addChapterComment = async (req, res) => {
    try {
        ...
        const comment = await prisma.chapterComment.create({
            data: { chapterId: chapterId, userId, content: content }
        });
...
export const deleteChapterComment = async (req, res) => {
    try {
        const commentId = getStringParam(req.params.commentId);
        const userId = req.userId;
        const comment = await prisma.chapterComment.findUnique({ where: { id: commentId } });
        if (!comment)
...
export const toggleFollow = async (req, res) => {
    try {
        ...
        if (!story)
            return res.status(404).json({ error: 'Story not found' });
        if (story.ownerId === userId)
            return res.status(400).json({ error: 'Cannot follow your own story' });
