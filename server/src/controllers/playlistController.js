...
const canAccessPlaylist = async (playlistId, userId) => {
    ...
    if (!playlist)
        return { allowed: false, error: 'Playlist not found', status: 404 };
...
export const reorderEntries = async (req, res) => {
    try {
        const { id } = req.params;
        const userId = req.userId;
        ...
        if (playlist.userId !== userId)
            return res.status(403).json({ error: 'Forbidden' });
        await prisma.$transaction(orders.map((o) => prisma.playlistEntry.update({
            where: { id: o.id },
            data: { order: o.order }
        })));
...
export const toggleFollow = async (req, res) => {
    try {
        ...
        if (!playlist)
            return res.status(404).json({ error: 'Playlist not found' });
        if (playlist.userId === userId)
            return res.status(400).json({ error: 'Cannot follow own playlist' });
