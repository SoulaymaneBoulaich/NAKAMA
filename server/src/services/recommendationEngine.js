 ...
 */
export const recordInteraction = async (userId, animeId, interactionType) => {
    try {
 ...
 */
const updateTagAffinity = async (userId, animeId, weight) => {
    try {
        ...
        if (tags.length === 0) {
            ...
            const newTags = [];
            if (animeData.genres)
                newTags.push(...animeData.genres.map((g) => g.name));
            if (animeData.themes)
                newTags.push(...animeData.themes.map((t) => t.name));
            if (animeData.studios)
                newTags.push(...animeData.studios.map((s) => s.name));
