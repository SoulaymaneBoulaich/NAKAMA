...
router.get('/similar/:animeId', async (req, res) => {
    try {
        ...
        if (tags.length === 0) {
            ...
            const newTagNames = [];
            if (animeData.genres)
                newTagNames.push(...animeData.genres.map((g) => g.name));
            if (animeData.themes)
                newTagNames.push(...animeData.themes.map((t) => t.name));
            if (animeData.studios)
                newTagNames.push(...animeData.studios.map((s) => s.name));
        ...
        for (const id of sortedIds) {
            try {
                ...
                results.push({
                    ...
                    tags: data.genres.map((g) => g.name).slice(0, 3)
                });
