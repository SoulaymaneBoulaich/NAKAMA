...
export const setupArenaSocket = (io, socket) => {
    ...
    socket.on('submit-argument', async ({ arenaId, content, userId, team }) => {
        try {
            ...
            if (!arena || !state || state.activeTeam !== team)
                return;
...
const startTimer = (io, code, arenaId, seconds, team) => {
    stopTimer(code);
    const state = {
...
//# sourceMappingURL=arenaSocket.js.map